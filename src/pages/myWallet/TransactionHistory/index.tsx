...
export default function TransactionHistory(
  // `showExport` gates the Export-CSV control: only the full-page Transactions
  // view (TransactionsPage) passes it; the compact dashboard embed omits it.
  { refreshSignal, showExport = false }: { refreshSignal?: number; showExport?: boolean } = {}
) {
  ...
  // Extension-style history filter: All / ↓ Received / ↑ Sent.
  const [txFilter, setTxFilter] = useState<"all" | "in" | "out">("all");
  const filteredHistory = React.useMemo(() => {
    if (txFilter === "all") return transactionHistory;
    return transactionHistory.filter((tx: any) =>
      txFilter === "in" ? !(tx.approx_float_amount < 0) : tx.approx_float_amount < 0
    );
  }, [transactionHistory, txFilter]);
  const pickFilter = (f: "all" | "in" | "out") => {
    setTxFilter(f);
  };
          ...
          coreBridgeInstance.hostedMoneroAPIClient.AddressTransactions_returningRequestHandle(
            ...
            function (
              ...
              if (err) { // already logged
                console.log("err:", err);
                // Leave the initial 'initiat' loading state so the full-screen
                // spinner overlay can't block the dashboard forever when the
                // server is unreachable (the header shows the connection state).
                setTransactionHistory((prev: any) =>
                  prev?.[0]?.status === "initiat" ? [] : prev
                );
                return
              }
  ...
  return (
    <Box
      sx={{
        ...
        padding: "20px",
        borderRadius: "0px",
        // Fill the tab frame so only the list inside scrolls (extension
      ...
      {transactionDetails.length > 0 ? (
        ...
        <>
          ...
          >
            ...
            {showExport && transactionHistory.length !== 0 && (
          ...
          {/* Extension .chips: hairline-bordered filter toggles, active = green */}
          <Box mt={1.5} sx={{ display: "flex", gap: "6px" }}>
            {([
              ["all", "All"],
              ["in", "↓ Received"],
              ["out", "↑ Sent"],
              ...
              <Box
                ...
                sx={{
                  ...
                  borderColor: (theme: any) =>
                    txFilter === key ? theme.palette.primary.main : theme.palette.divider,
                  color: (theme: any) =>
                    txFilter === key ? theme.palette.primary.main : theme.palette.text.secondary,
                }}
              >
                {text}
              </Box>
          ...
          <Box mt={2}
            sx={{
              // The list gets exactly the vertical space left in the card;
              // its own content scrolls (extension .history-card).
              flex: 1,
              minHeight: 0,
              overflowY: "auto",
              padding: "0 10px",
              ...
              '&::-webkit-scrollbar-thumb': {
                background: (theme: any) => theme.palette.mode === 'dark' ? '#2a2a2a' : '#D1D1D1',
                borderRadius: '0px',
              },
