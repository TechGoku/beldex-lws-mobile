...
export default function SignIn() {
  ...
  const validateComponentsForLogin = () => {
    try {
      ...
      // The seed is cryptographically valid locally, so enter the wallet
      // immediately rather than blocking on the LWS login network call. That
      // call hangs when the server/domain is unreachable (e.g. a filtered ISP)
      // and used to strand the user on a full-screen spinner with no route to
      // Settings. The dashboard registers + syncs once reachable, and shows a
      // "No connection" state otherwise.
      setLoading(false)
      navigate("/mywallet");
      ...
      {!showSignWithKey && (
        ...
        >
          ...
          >
            ...
            {showErrMsg && (
              <Typography
                sx={{ color: "#ff5c5c", fontWeight: 400, marginTop: "5px" }}
              >
                {errMsg}
              </Typography>
            )}
