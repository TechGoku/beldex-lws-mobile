// BNS name resolution for the send flow (ported from the browser extension).
//
...
// visible to the explorer — standard for light wallets. On device the request
// rides the native HTTP stack (CapacitorHttp), so no CORS constraints apply.

const BNS_LOOKUP_URL = "https://explorer.beldex.io/api/bnslookup";
...
async function fetchWithRetry(url: string): Promise<Response> {
  // The explorer drops bursty connections (rate limiting), which surfaces as
  // an instant "Failed to fetch" — one short-backoff retry rides it out.
  try {
...
async function lookup(name: string): Promise<LookupResult> {
  const res = await fetchWithRetry(`${BNS_LOOKUP_URL}?name=${encodeURIComponent(name)}`);
 ...
 * without the ".bdx" suffix: the typed extension is stripped first, then the
 * canonical registered form ("<name>.bdx" — measured against the live
 * registry) is queried, with the bare form as a fallback. Returns null when
 * the name isn't registered at all; throws a descriptive error when it's
 * registered but carries no wallet mapping.
 */
export async function resolveBnsWallet(input: string): Promise<string | null> {
  ...
  for (const candidate of [`${bare}.bdx`, bare]) {
